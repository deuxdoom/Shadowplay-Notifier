...
시계의 콜론만 깜빡이고 초는 계속 표시되는지, 배경을 다시 칠해도 박자와 숫자
위치가 유지되는지, 별도 타이머나 도형이 늘지 않는지도 확인합니다.
...
`python test/test_startup.py`는 별도의 임시 레지스트리 키에서 로그인 자동 실행의
등록·해제, 경로 변경, 공백·한글 경로, 쓰기 실패를 검사합니다. 실제 자동 실행
등록은 변경하지 않습니다.
